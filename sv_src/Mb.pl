...
#★これは何？
# まちBBSのofflaw.cgiを使用して、2ch形式のDATとしてまちBBSのログをキャッシュするものです。
...
#2c.plとは違いオプションで1番目のレスを表示する事は出来ません。
...
# 保存されるDATの仕様は、2c.plの物と全く同じでファイルの１行目にLast-Modified\tデータの行数(\tはタブ)
...
#・DATの差分読み込みに対応。
#08/07/03 ver1.3
#・If-Modified-Sinceを送ることにより新レスがなければDATを作り直さないようにした。
#08/06/20 ver1.2
#・mhttp.pl内のあぼーん処理がうまく出来ていなかったので修正。
#　それに伴いあぼーん用ロジック改良の結果、少量の高速化。
#08/06/15 ver1.1
#・Mb.pl高速化の為、Mb.pl専用のhttp.plであるmhttp.plを作成。
#  mhttp.plの方でまちBBSの形式を2ch形式にするようにした。
#  これによりかなりの高速化及び負担減。
#08/06/06 ver1.0
#・作成。
...
RELOADBRD:
				...
				if($mhttp'header[0] =~ / 304 /){	#キャッシュを使用する(304 Not Modified)
					$readcache = 1;
				} elsif($reload == 0 && ($mhttp'header[0] =~ / 302 / || ($mhttp'header[0] =~ / 20[0-9] / && length($str) <= 100))){	#板移転(302 Found or 200 OK && $str eq "")
