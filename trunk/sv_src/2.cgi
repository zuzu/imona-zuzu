		...
		if($iboard !~ /100/){
			if(length($resbuf[3]) =~ /9/){
				...
				if($resbuf[3] =~ /0$/){	                    #idが0な場合
					$resbuf[3] = "$resbuf[3]      Type:PC&Other"; #idにPC to Otherを追加
				}
				if($resbuf[3] =~ /O$/){	                                #idがphone to PHSな場合
					$resbuf[3] = "$resbuf[3]      Type:mobilephone";#idにmobilephone&PHSを追加
				}
				if($resbuf[3] =~ /P$/){	                           #idがPな場合
					$resbuf[3] = "$resbuf[3]      Type:p2.2ch.net";#idにp2.2ch.netを追加
				}
				if($resbuf[3] =~ /Q$/){	                           #idがQな場合
					$resbuf[3] = "$resbuf[3]      Type:middle2chbrowser";#idにmiddle2chbrowserを追加
				}
				if($resbuf[3] =~ /o$/){	                           #idがoな場合
					$resbuf[3] = "$resbuf[3]      Type:AIR-EDGEPHONE";#idにmiddle2chbrowserを追加
				}
				if($resbuf[3] =~ /I$/){	                           #idがIな場合
					$resbuf[3] = "$resbuf[3]      Type:DoCoMo";#idにDocomoを追加
				}
				if($resbuf[3] =~ /i$/){	                           #idがIな場合
					$resbuf[3] = "$resbuf[3]      Type:DoCoMo";#idにDocomoを追加
				}
